...
- Consider human participants not just as dynamic obstacles, but also as distributed sensors.
...
- As an MDP, it is meant for **sequential decision process**, seeking optimal policies for **long-term objectives**, contrary to reactive approaches. It is for instance beneficial for highway exit cases where a long planning horizon is required.
...
**Reward functions** are usually some combination of several terms. It should encourage the agent to _reach the goal_, _avoid collisions_, _avoid deviation from a predefined path_ or _from a desired speed_, consider _passenger comfort_, consider _social behaviour_ ... The question is then **how to weight the different contributions**. This can be very challenging, as acknowledged by (Schratter et al. 2019) where a tuning of the reward parameters is performed.

One could use **inverse reinforcement learning** (IRL) to determine appropriate values for the reward function. (Sun et al. 2019) learn the cost function of an MPC via IRL, but I did not see any IRL applications for POMDPs. For sure it will be done in future works.
...
It is not always possible to represent **directly their probability distributions**. A commonly used approach is to use a **generative model** instead. From a given state and a given action, it generates a new hidden state (**transition** model) and observation (**measurement** model).

The transition model usually holds **one part for the ego vehicles** whose state is fully known and **another part for the dynamic obstacles**. For the ego information, the deterministic _bicycle kinematic model_ of (Kong et al. 2015) is widely used to derive the new position, based on the current position, the current vehicle and the selected acceleration.
...
Another possibility is to infer the action of other participant, by assuming they follow the Intelligent Driver Model (IDM). IDM is a _crash-free microscopic car-following model_ that is widely applied for driver intention estimation. It can be used to **model how others will react** to actions of the ego vehicle, by estimating their **longitudinal control responses**.

Besides point-mass models and probabilistic approaches, (Schratter et al. 2019) uses a simple **reachability** model. Depending on the current pedestrian state, future positions for the pedestrian are computed based on a **set of possible acceleration values**.

### POMDP Solvers

Solving the POMDP means **finding the optimal policy**, i.e. the function that tells which action to take given any state to **maximize some notion of cumulative reward**. As a probabilistic non-linear optimization problem under state uncertainty, it is hard to solve, and many solvers rely on the **approximation of the optimal policy**.
...
| ![Example of the construction of a policy tree in an online solver. [Source](https://papers.nips.cc/paper/5189-despot-online-pomdp-planning-with-regularization.pdf)](media/pics/somani_despot_tree.PNG "Example of the construction of a policy tree in an online solver. [Source](https://papers.nips.cc/paper/5189-despot-online-pomdp-planning-with-regularization.pdf)")  |
|:--:|
| *Example of the construction of a policy tree in an online solver. [Source](https://papers.nips.cc/paper/5189-despot-online-pomdp-planning-with-regularization.pdf)* |

The concept of **_tree_** is important in **online** solving. From one **initial state** (or belief on it), one could **consider all the possible actions** and sample from the transition model to derive new states (and new believes on them). One option could then repeat the process to **expand the policy tree up to a given horizon**. Finally, the **reward collected on each path** in this tree could be counted and the first action on the best path can be selected. This is a very simplified (and non-efficient) description of the idea of main online methods. This can be done using **Monte Carlo Tree Search** ([MCTS]( https://en.wikipedia.org/wiki/Monte_Carlo_tree_search)). This type of algorithms is particularly appropriate for **online decision-making**. Here are some advantages:
...
- The term _reinforcement learning_ appeared in `41` of the released papers (`12`%). Among them, `11` were **presenting an implementation** while the others where referencing RL-related works in their literature review.
...
- If the transition and reward models are known, the solving task is referred to as planning.
  - Planning is attractive since it **does not require any interaction with the MDP environment**. But the **design of a realistic transition model** is challenging. Given some acceleration and steering command, deriving the position of the ego vehicle at the next step is feasible. But defining the response of other drivers to these decisions is way more difficult.
- If not, it is called (reinforcement) learning.
  - To collect experience for learning, RL requires an MDP environment to interact with. The environment can be a **simulator** (most approaches) or the **real-world**. [WAYVE](https://wayve.ai/blog/learning-to-drive-in-a-day-with-reinforcement-learning) has demonstrated impressive performance with the later technique. But the problem of **_sample efficiency_**, i.e. the amount of experience the agent needs to generate in an environment during training to reach a certain level of performance, advocates for the **use of a simulator**. This is especially true for **model-free approaches** which need a lot of experience data to derive a good policy. Not to mention the **safety considerations**.
...
Earlier this year, (J. Chen, Yuan, and Tomizuka 2019) tested three state-of-the-art model-free deep RL algorithms: _DDQN_, _TD3_ and _SAC_. The task was to **navigate through a roundabout** and take the third exit. The three approaches where tested `50` times. As shown on their results, **DDQN and TD3 could not reach the goal point one single time**.
...
(Bouton et al. 2019) finds that using a **pure RL technique does not enable the agent to act more safely** than when using simple **rule-based methods**. But when the **scenario becomes more complex**, with a flow of cars and pedestrians, the benefit of using RL over a rule-based method becomes clear.

Among successful approaches, only few report **evaluations on real vehicles**. (Folkers, Rick, and Christof 2019) conducted test on a full-size research vehicle and show the potential of their RL-based controller for autonomous exploration of a parking lot. It would have been nice to have demonstrations from them or from the RL research teams as [WAYVE](https://wayve.ai/blog) during the track-demo day!
...
On the other hand, learning-based approaches have shown promising results in decision modules. But:
...
- They can **be hard to adjust manually**. For instance, **fixing MPC errors on the fly** is easier than fixing a model-free RL, which requires **tuning the reward function** and then restarting the training.
...
The **need for such combination** is illustrated in (Schulz et al. 2019). A probabilistic mapping from context and route intention to action distribution is learnt with deep neural networks. The benchmark includes a non-learnt approach. They show that for **short-term predictions** of up to around `2` seconds, all the **learning-based models outperform the rule-based action model**. But the rule-based model becomes **more accurate for predictions with very long horizons**, exceeding `15s`.
...
It reminds me the issue of _drift_ in imitation learning, when error accumulates fast in a trajectory, and this **brings the agent into situations that it never dealt with before** (_cascade errors_). A common solution it then to use _Dataset Aggregation_ methods like [DAgger](https://arxiv.org/abs/1011.0686). Or in this case maybe to **combine the rule-based with the learning-based model**.

I have already listed some example of successful combinations in the [RL sections](#combining-learning-and-planning). The benefits can relate to the **generalization capability**, the **interpretability gain** or the **safety**. Here is another example.
...
I saw several learning-based approaches to **replicate the behaviour of a non-learning-based methods** such as probabilistic or kinematics-based approaches. And they show promising results, especially in **high complex interactions**.
...
In their conclusion, the authors suggest investigating the idea of a **unified framework that combines _Seq2Seq_ models** (learning-based) with **probabilistic graphical models**. Again, some fusion seems beneficial.
...
This remark reminded me the work of (Koenig, Rehder, and Hohmann 2017), who **preferred to use BNs due to the high safety demands in AD**, even though they were **outperformed by DNN in some of their experiments**.
...
- Uncertainty in the **intent** of other traffic participants. More generally this encompasses all the **social behavioural uncertainties**, such as local driving preferences.
...
He then spent a long time discussing about uncertainties in **maps** and **localisation**. Maps contain information used for **tactical long-horizon decision making** such as the possible actions that one can take at a crossing or simply the speed limitation. But if maps **do not get updated** as the environment changes, it can be **very misleading**.
...
While most of these methods for perception and decision-making **output an uncertainty measure**, they treat the information from localisation and perception as **point estimates** (with zero variance). The **propagation of uncertainty measurements through all the AD modules** would be preferable.

For that purpose, the **environment representation** based on **probabilistic occupancy grids** used in (Barbier et al. 2019) seems appropriate to capture the perception uncertainty.
...
The POMDP formulation can address this issue. Being a **belief state planning** problem, it allows for **decision making under uncertainty**. (Schörner et al. 2019) use a POMDP to tackle uncertainty in occlusion. Similarly, (Hubmann et al. 2019) present an **occlusion-aware POMDP** (OCAPOP) showing that:
...
The vehicle sometimes decides to **postpone the decision** about crossing the intersection and prefers increasing its _fov_ by **taking a lateral offset to capture more information**. This strategy, called **"information gathering"**, allows for less conservative behaviour.
...
It can be noted that the resulting planner can drive nearly the same trajectories as an **omniscient planner would**.
...
During his keynote, Christoph Stiller called for more research in **_“Cooperative perception, behaviour and planning”_**. According to him, V2V techniques hold a huge potential to improve safety, reduce emission and increase the traffic flow efficiency.
...
Instead of describing the state of other participants with a vector and then **combining all the instances in one joint state vector**, **images representing the whole scene** can be used. This is done in (Joonatan and Folkesson 2019), where a generic visual representation is designed. The main advantage of representing the traffic scenario with an image is that number of traffic participants does not affect the size of the state. Moreover the computational complexity can, in some cases, be kept constant. Yet the resolution of the generated paths is dependent on the resolution used in image, `128`x`128` single-channel in this case. Finally, the encoding of information such as the **semantic intention** may be tricky, especially if expressed with uncertainties.

Several **hybrid representations**, i.e. the combination of an image and vector of some features, were used at IV19.

In (Folkers, Rick, and Christof 2019), the state of the MDP is composed of an **ego feature vector** and an **obstacle grid**. In this cost map, also called _ternary perception map_, the target position is given as `−1`, free space is encoded by `0`, while obstacles and lane boundaries are indicated by `1`.
...
The idea of a **hybrid representation** is also used in (Pusse and Klusch 2019). The observation input for the IS-DESPOT solver contains, inter alia, an `84`x`84` image representing the intention of the other vehicles.
...
- In **Manoeuvre-based** models, traffic participants are assumed to execute their intended manoeuvres independently from other traffic. The task is to try to infer the underlying manoeuvre among a finite set of discrete manoeuvres (e.g. _lane keeping_ or _lane change_). The past trajectory or vehicle features are used for this classification problem. Again, interactions are not considered.
...
(Diehl et al. 2019) propose to use Graph Neural Network (GNN). Interactions are modelled as directed edges leading from the up to `8` neighbouring vehicles to the ego vehicle.
...
My next observation was that **conditional variational auto-encoder** (CVAE) and **generative adversarial imitation learning** (GAIL) were largely used for interaction-aware prediction.

**CVAE** was used for instance in (Hu, Zhan, and Tomizuka 2019). The idea is to **jointly predict motions for pairs of interacting agents**. But not all vehicles are interacting with each other. The idea is therefore to start by identifying the interacting pairs. To do that, intention probabilities are derived for each vehicle. Then, for each pair of vehicles, reference paths are used to **determine if a conflict point exists**. If not, their future trajectories will be independent from each other. For interacting pairs, their **joint motions are predicted using a CVAE**.

This idea of **first identifying interaction** and **then predict trajectories** is also found in (J. Li et al. 2019). They introduce a **hierarchical** structure for Coordination and Trajectory Prediction System (CTPS). First, a **macro-level recognizes the coordination** between the multiple interactive agents. This information, together with on historical states and the context information, is used by the **micro-level pattern prediction module** to generate motion hypotheses.
...
Most frequent crash scenarios can be retrieved from the National Highway Traffic Safety Administration (NHTSA) traffic crash data. The [NHTSA pre-crash typology](https://www.nhtsa.gov/sites/nhtsa.dot.gov/files/pre-crash_scenario_typology-final_pdf_version_5-2-07.pdf) is for instance used by the [CARLA AD challenge](https://carlachallenge.org/challenge/nhtsa/) to **test submissions on common critical scenarios**.
...
In many papers **real-world experiments are left for future works**. One reason it that it is difficult to **transfer models** derived from simulation or hand-crafted scenarios to real cars.
...
I do not address the question of **realistic simulators** here. Just mentioning one sentence that came several times at IV19:

> The chicken and egg problem: validating the simulator is probably harder than validating the algorithm itself.
...
Several collections of **scenarios recorded from real traffic** exist, as reviewed by (Kang, Yin, and Berger 2018) or listed by [scale.ai](https://scale.ai/open-datasets). But most entries **mainly cover perception**.
...
> “10 years ago, safety was often said to be for production, not for research. Now, it has become a research area.”
...
This topic was relatively new to me. I focus on the following points:
...
This question was addressed by Jack Weast, Vice President at Mobileye, in his keynote **_“An Open, Transparent, Industry-driven Approach to AV Safety”_**.
...
Another idea could be to **try to avoid crashes** at all cost. And to (try to) prove it to the authorities. But this is not realistic. Imagine you are surrounded by cars that are coming together close to you. **Avoiding accidents is just impossible**, as illustrated in the next figure.
...
The main point, as I understood, is that RSS is based on the **notion of "blame"**: *ensure that the AV does not initiate any dangerous situation*.
...
I will also remember a second idea of RSS: the model of **human implicit rules**.
...
- But **estimating the RSS parameters can be complicated**. How would you estimate the **maximum braking capability** of the vehicle in front of view? The same as yours? The average on the market? What if it is a sport car with higher braking capabilities? This value, usually denoted by `β max`, plays a significant role in the model since it conditions the safety gap distance.
...
As I understood, for a **particular reward function**, the **optimal value function (or *utility*)** of an MDP happens to be the **probability of reaching the goal safely**. This utility function can be learnt offline using value iteration. The values, in this case probabilities, are then used to learn the policy: the RL agent can **chose among the set of actions** that have a **probability of safety larger than a safety threshold**.
...
Before attending the SIPD workshop, I could assist to the panel discussion of the workshop on **_Ensuring and Validating Safety for Automated Vehicles_** ([EVSAV](https://www.ifr.ing.tu-bs.de/en/dept-vehicle-electronics/events/iv-workshop-2019)).

The main conclusion was: **The approach _Vision Zero_ promoted by most OEMs and Bay-area newcomers is misleading.**
...
Markus Maurer, from TU Braunschweig, deplored the absence of a **rational discussion about AD fatality risk**, arguing that most OEMs do not want to have a public debate about it. “We all know that we must minimize risk. But **it won’t reach zero**”. Engineers cannot decide on themselves. Decisions should rather be discussed with the authorities and need to be **backed-up by the society**.

To show that **_risk zero_ is just not realistic**, Katherine Driggs-Campbell drew a parallel with the **introduction of automation in aviation** during the SIPD workshop. Introducing autonomy is supposed to be safer. But as shown by [Dominique Chatrenet](http://ec.europa.eu/invest-in-research/pdf/workshop/chatrenet%20_b3.pdf), for new piece of automation that is introduced we see an **improvement in the long term** but a **spike in fatalities in the transient period**. Maybe _Vision Zero_ should be renamed _Vision Asymptotically Zero_ 😉
...
But before going to that, one could argue that such a debate did not take place in the **aviation domain**. It raises an alternative question: _To what extend should engineer should communicate to the public?_ _Should we know everything?_ _Should some details be kept hidden?_ Airplane manufacturers do not explicitly communicate on risk. Or at least it is not covered in the media. But **we trust it**. The difference with the vehicles on the street is probably that not everyone is a pilot and that everyone will compare the AD decisions with what he/she would have done instead.

> "Acceptance is actually not based on risk. But rather on expectation of benefits, especially in entertainment."

Another participant brought another interesting point: **social acceptance is not (only) corelated to risk**, but rather **to the expectation of benefits**. He then drew a parallel with the development of smartphones. They always have been presented as _cool_. From the beginning, their threats did **not carry much weight** in the face of the ability to watch video from everywhere, to access the web remotely and to increase one’s “social” connections. And there has been no public discussion at all.

Today, AD is mostly in the **hand of industrials** and under the **eyes of regulators**. At the same time, **over-optimistic announcements are generating high expectations**. And AD is presented as “cool”. There is probably no room for any public discussion.

### Social Benefit

I found the keynote **_Public policy challenges of new mobility services_** by **Tom Vöge**, transport policy expert, was very appropriate. Not only to make a **break with scientific concepts** and algorithms. But also to **step back** and question our (excited) involvement in developing this technology. An **invitation to come down from the ivory (research) tower** and **question the social benefit** of AD.
...
As [stated](https://www.nature.com/articles/d41586-019-01473-3) by Ashley Nunes, multiple influential groups participate to the promotion of AD. "**Economists** describe how the technology will **upend trade**. **Urban planners** describe how our cities will soon look very different after they have been reshaped, owing to the **diminished need for car parks**. **Medical professionals** proclaim the **end of road-traffic accidents** as we know them." Other sources also mention the mobility offered to people that are not in a state to drive. And the possibility to _let the car_ **take over in situations where driving is all but fun**. One would also want to add _“capturing brain time to offer entertainment, expose advertisement or impose remote work”_.

Then come the threats. The main point of this keynote was about **share mobility** and **threat of congestion**.

> “If it leads to more sharing and more people to car, it is a good idea. Otherwise, there is no benefit at all. In Europe, commuting to/from work has an occupancy rate of 1.2. If could be 2, 3, 4, 10. Or it could become 0. That would be a disaster.”
...
Several **heavy rainfalls** rhythmed the day. Fortunately, we could seek shelter under tents, while IAV’s demo was interrupted to protect the sensors placed on the roof. Meanwhile, the KIT car could keep driving under the rain but must then stop as the sun reappeared because  **reflexions on the road** were fooling its **camera-based localisation**.
...
A research team from KIT also demonstrated perception and planning in presence of VRUs. It **detected a bicycle** in the right lane and decided to **overtake it, going to the opposite lane**.
